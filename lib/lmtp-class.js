...
class SMTPBackendServer extends SMTPServer {
    ...
    constructor(lmtp_conf,endpoint_conf) {
        ...
        this.pesistence = false;  // for attachments
     ...
     * @returns string -- base64url encoded blob with URL
     */
     ...
     * extract_addr
     * 
     * Retur the first address of the `from` field, which is output by mailparser
     * 
     * @param {object} from 
     * @returns 
     */
    ...
    /**
     * convert_to_persistence_object
     * 
     * @param {object} attch - the attachment descriptor from mailparser
     * @param {object} mailobj - body and headers from mailparser
     * @returns 
     */
    ...
    async relay_attachments(mailobj) {
        ...
        for ( let attch of attachments ) {
            ...
            if ( p_object ) {
                id_list_promises.push(this.pesistence.new_entry(p_object))  // send the attachment to persistence
            }
        }
